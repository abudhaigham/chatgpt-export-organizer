...
- Add evidence-based extraction for `.dat` assets using file signatures and container
  inspection instead of relying on filename extensions.
- Organize extracted files by detected type and write an `Asset_Inventory.csv` audit
  inventory with checksums, references, and cautious origin classifications.
- Add `--extract-assets [DIRECTORY]` with the safe default output directory
  `ChatGPT_Files`.
- Integrate typed asset extraction into the managed-import desktop workflow.
- Add a refreshed Arabic desktop interface with a consistent visual identity, clearer
  processing stages, improved dark-mode contrast, and a visible operation log.
- Add permanent pytest source-path configuration for reliable repository test runs.

### Safety

- Preserve earlier managed imports and generated results; every new import continues to
  use an isolated directory.
- Treat role associations as evidence only. They do not prove whether a file was uploaded
  by the user or generated by ChatGPT.
- Keep extraction copy-based and idempotent; source `.dat` files remain unchanged.
